...
class RAWGamesAPI(RequestInstance):
    ...
    def get_games_list_last_year(self):
        today = datetime.today()
        days_in_year = 365
        one_year_before = today - timedelta(days=days_in_year)
        two_years_before = today - timedelta(days=2*days_in_year)
        return self.get_games_list(two_years_before, one_year_before)

    def get_games_list(self, date_since, date_to):
        y_s = date_since.year
        m_s = date_since.month
        y_t = date_to.year
        m_t = date_to.month
        return self.get("/games", {"dates": f"{y_s}-{m_s:02d}-01,{y_t}-{m_t:02d}-01"})
